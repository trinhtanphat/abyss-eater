...
test('parseClientMessage accepts only versioned bounded input messages', () => {
  const parseClientMessage = requireFn('parseClientMessage');
  assert.deepEqual(
    parseClientMessage('{"type":"input","v":1,"seq":2,"dir":{"x":1,"y":0,"z":-0.5}}'),
    { ok: true, message: { type: 'input', v: 1, seq: 2, dir: { x: 1, y: 0, z: -0.5 } } },
  );
  assert.deepEqual(
    parseClientMessage('{"type":"input","v":2,"seq":2,"dir":{"x":1,"y":0,"z":0}}'),
    { ok: false, code: 'bad_version' },
  );
  assert.deepEqual(
    parseClientMessage('{"type":"input","v":1,"seq":-1,"dir":{"x":1,"y":0,"z":0}}'),
    { ok: false, code: 'bad_seq' },
  );
  assert.deepEqual(
    parseClientMessage('{"type":"input","v":1,"seq":1,"dir":{"x":1,"y":null,"z":0}}'),
...
test('parseClientMessage validates ping, malformed JSON, unknown types and size', () => {
  const parseClientMessage = requireFn('parseClientMessage');
  assert.deepEqual(parseClientMessage('{"type":"ping","v":1,"t":123.5}'), {
    ok: true,
    message: { type: 'ping', v: 1, t: 123.5 },
  });
  assert.deepEqual(parseClientMessage('{'), { ok: false, code: 'bad_json' });
  assert.deepEqual(parseClientMessage('{"type":"chat","v":1}'), { ok: false, code: 'bad_type' });
  ...
  assert.deepEqual(parseClientMessage('{"type":"ping","v":1,"t":"123"}'), { ok: false, code: 'bad_ping' });
});
