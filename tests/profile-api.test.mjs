...
test('Worker source exposes bounded persistent guest session and profile APIs', () => {
  ...
  assert.ok(worker.indexOf("url.pathname === '/api/profile'") < worker.indexOf("url.pathname === '/ws'"), 'profile API must stay outside the GameRoom hot path');
});

test('production build embeds Carrier 3 pure modules without changing protocol v2', () => {
  ...
  assert.ok(bundle.includes("url.pathname === '/api/session'"));
