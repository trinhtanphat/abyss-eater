...
- Keep `qs3d.site` unchanged because its zone is in a different Cloudflare account.
...
- [ ] Write failing tests covering input normalization, bounded movement, size/speed scaling, valid/invalid eating thresholds, food growth, and respawn reset.
- [ ] Run `node --test tests/game-logic.test.mjs` and confirm RED because the module does not exist.
- [ ] Implement the smallest pure game-rule module that passes those tests.
- [ ] Run `node --test tests/game-logic.test.mjs` and confirm GREEN.
...
- [ ] Write a failing build test requiring the production bundle to contain `GameRoom`, `acceptWebSocket`, `/health`, `/ws`, and all four public assets.
- [ ] Run the build test and confirm RED because the builder/template is absent.
- [ ] Implement the Worker template and dependency-free build assembler.
- [ ] Build `dist/worker.mjs` and run the build test to GREEN.
...
- [ ] Add a failing build assertion that production output embeds the pinned Three.js URL, HUD markers, touch controls, and WebSocket endpoint.
- [ ] Run tests and confirm RED.
- [ ] Implement procedural ocean/fish/plankton rendering, interpolation, camera follow/zoom, keyboard and touch input, HUD, connection/reconnect behavior.
- [ ] Rebuild and run the full tests to GREEN.
...
- [ ] Add package scripts and CI workflow using Node 22.
- [ ] Document architecture, local validation, controls, Cloudflare target account and cross-account domain limitation.
- [ ] Run `npm test` and `npm run build` from a clean checkout-equivalent directory.
- [ ] Commit local implementation with descriptive commits.
...
- [ ] Upload the module Worker with a Durable Object binding and new SQLite class migration.
- [ ] Confirm `workers.dev` is enabled for the Worker.
- [ ] Fetch `/health` and `/` from the deployed URL and verify success.
- [ ] Open a WebSocket connection probe if available; otherwise verify deployment metadata and HTTP routing.
- [ ] Leave `qs3d.site` DNS untouched because the zone belongs to another account.
